...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SentientWeb consent store | SentientWeb | First-party (`sentientwebsite.com`) | Necessary / preference | After user chooses | Forces analytics off when GPC asserted | Choices + timestamps | Browser controlled | Footer Privacy choices | `src/privacyPreferences.ts` + `ConsentManager.tsx` |
| Homepage visual media and integration marks | SentientWeb | First-party (`sentientwebsite.com`) | Decorative / marketing media | Yes | N/A | Standard HTTP request metadata for first-party static assets | Edge/server log defaults | Browser controls | First-party files under `/media/` and `/logos/` |
