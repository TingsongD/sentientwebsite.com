...
function useGlobalPrivacyControl() {
  const [globalPrivacyControl, setGlobalPrivacyControl] = useState(false)

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- browser-only privacy signal must not affect SSR markup
    setGlobalPrivacyControl(readGlobalPrivacyControl())
  }, [])
...
export function ConsentManager() {
  ...
  const globalPrivacyControl = useGlobalPrivacyControl()
        ...
        {showDetails ? (
          <div className="mt-4 grid gap-3">
            ...
            <Toggle
              ...
              checked={globalPrivacyControl ? false : draft.analytics}
